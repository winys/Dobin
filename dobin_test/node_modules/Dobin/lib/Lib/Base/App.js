...
module.exports = {
	...
	execAtion : function ( controller, actionFn, data ) {		
		try{
			...
			var res = actionFn.apply( controller, data );
			if($.isGenerator (res)){
				co(
				...
				.catch(
					function(err){
						/*if( !controller.resend ){
							controller.echo(err.toString());
							controller.end();
						}*/
						console.log(err);
						$.app_log(err.toString());
					}
				);
			}
			else{
	...
	exec : function ( http ) {
		...
		http.header('X-Powered-By','Dobinjs');
		...
		if( http.isResource ){
			...
			if (fs.existsSync(file)) {
				var contentType = mime.lookup(file),
					...
					;
				http.setHeader('Content-Type', contentType + '; charset=' + $.Config('encoding'));
			...
			if ( !$.isFunction(actionFn) ){
				$.app_log( "Can't find Action with `"+http.action+"`");
			...
			var BaseController =  require($.DOBIN_PATH+"/Lib/Base/Controller.js");
			BaseController.init(http);
			//先执行 init 方法
			if( $.isFunction(controller['init']) && !BaseController.resend)
				this.execAtion( 
					BaseController,
					controller['init'] ,
					this.getParams( controller['init'], http )
				);
			if( !BaseController.resend )
				this.execAtion( BaseController, actionFn, data);
		}
